...
`gamekit-rs` v0.2.0 targets the GameKit APIs that are practical to expose from a safe Rust surface on macOS and mirrors the `screencapturekit-rs` bridge layout:
...
| Save | `GKSavedGame` list, load data, save, delete, resolve conflicts | `SavedGame` | Broad coverage; listener registration is not wrapped |
...
- `GKChallengeDefinition` is compiled only when the detected macOS SDK is 26 or newer.
- At runtime, the bridge returns `GameKitError::Unavailable` when a gated API is called on an older SDK or OS.

## Remaining known gaps

The crate is substantially broader in v0.2.0, but it is not a literal 100% wrapper of every Apple symbol. The main intentionally-unwrapped areas are:

- image/photo loading APIs
- listener registration APIs that require long-lived Objective-C delegate objects beyond the current crate scope
- newer matchmaking queue and nearby-player helpers
- a few access-point and challenge-definition convenience helpers added in the latest SDKs
